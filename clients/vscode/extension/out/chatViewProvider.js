...
class ChatViewProvider {
    ...
    clientVersion;
    sessionLog;
    static viewType = "hitl.chatView";
    webviewView;
    conversationHistory = [];
    constructor(extensionUri, apiClient, getSession, clientVersion, sessionLog) {
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        if (session) {
            this.postMessage({
                type: "sessionStatus",
                armId: session.armId,
                landmark: (0, sessionState_1.currentLandmark)(session),
    ...
    async handleUserMessage(text) {
        ...
        try {
            ...
            await this.sessionLog.logInteractionOk(session.sessionId, session.projectId, session.armId, session.interactionCount);
            ...
            await this.sessionLog.logInteractionError(session.sessionId, session.projectId, session.armId, session.interactionCount, errMsg);
            this.postMessage({ type: "setLoading", loading: false });
            this.postMessage({
                type: "error",
